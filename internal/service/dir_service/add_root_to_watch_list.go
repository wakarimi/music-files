...
func (s *Service) AddRootToWatchList(tx *sqlx.Tx, dir model.Directory) (createdDir model.Directory, err error) {
	...
	if !directoryExists {
		...
		return model.Directory{}, errors.NotFound{Resource: "directory on disk"}
	}
	...
	if alreadyTracked {
		...
		return model.Directory{}, errors.Conflict{Message: err.Error()}
	}
...
func (s *Service) getContainedRoots(tx *sqlx.Tx, absolutePath string) (containedRoots []model.Directory, err error) {
	log.Debug().Str("path", absolutePath).Msg("Getting root directories that are subdirectories of the current one")

	containedRoots = make([]model.Directory, 0)
...
func (s *Service) addIntermediateDirs(tx *sqlx.Tx, containedRoots []model.Directory) (err error) {
	...
	for _, containedRoot := range containedRoots {
		...
		for !alreadyTracked {
			newDirId, err := s.DirRepo.Create(tx, model.Directory{
				ParentDirId: nil,
				Name:        currentPath,
			})
			if err != nil {
			...
			err = s.DirRepo.Update(tx, prevDirId, model.Directory{
...
func (s *Service) getEqualTrackedDir(tx *sqlx.Tx, absolutePath string) (equalTrackedDir model.Directory, err error) {
